...
public class DaoProductoImpl implements DaoProducto {
	 ...
	 */
	@Override
	public List<Producto> listProductos(Integer... ids) throws SQLException, NamingException{
		try(Connection connection = this.conexion.getConnection()){
			if(ids.length == 0)
				...
				return new QueryRunner()
						.query(connection,
								"SELECT * FROM productos WHERE id_producto IN " + sql, 
								new ArrayListHandler())
						.stream()
						.map( rs -> new Producto(
								(Integer)rs[0],//ID
	 ...
	 */
	@Override
	public Producto getProducto(Integer id) throws SQLException, NamingException{
		try(Connection connection = this.conexion.getConnection()){
			List<Producto> temp = new QueryRunner()
					.query(connection,"SELECT * FROM productos WHERE id_producto = ? ", new ArrayListHandler(),id)
					.stream()
					.map( rs -> new Producto(
							(Integer)rs[0],//ID
							(String)rs[1],//CODIGO
	 ...
	 */
	@Override
	public Producto getProducto(String codigo)  throws SQLException, NamingException{
		try(Connection connection = this.conexion.getConnection()){
			List<Producto> temp = new QueryRunner()
	...
	public int insertProducto(Producto producto, Connection connection) throws SQLException{
		try(PreparedStatement pst = connection.prepareStatement(
				"INSERT INTO productos (codigo,descripcion,origen,destino,cliente_envio,cliente_recepcion,id_estado) " +
				"values (?,?,?,?,?,?,?) ")){
